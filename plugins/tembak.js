let handler = async (m, { conn, text, usedPrefix }) => {
 ...
 const nama = conn.getName(m.sender)
  ...
  if (!text && !m.quoted)
    ...
    return conn.reply(m.chat, `${nama} mau nembak siapa`, m);
    ...
    if (
      ...
      if (global.db.data.users[pacar].pasangan == user) {
        ...
        conn.reply(
          ...
          `Tau sopan santun dikit teman\n@${
            ...
            pacar.split("@")[0]
          }\n\nSilahkan cari pasangan lain aja kntol jan ganggu hubungan orang!\ndenda : ${format(
            denda
          )} (10%)*`,
          m,
          {
      ...
      const p = m.sender
        ...
        conn.reply(
          ...
          ` @${
            user.split("@")[0]
          } telah ditembak oleh @${p.split("@") [0]} \n\nSilahkan tunggu jawabannya ya!\n@${
            user.split("@")[0]
          }, silahkan ketik ${usedPrefix}terima @${p.split("@") [0]} atau ${usedPrefix}tolak @${p.split("@") [0]}\n\nuntuk memberi jawaban`,
          m,
          {
            contextInfo: {
              mentionedJid: [user],
      ...
      conn.reply(
        m.chat,
        `Selamat nte resmi berpacaran dengan @${
          user.split("@")[0]
