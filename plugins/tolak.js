...
const nama = conn.getName(m.sender)
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `*${nama} mau nolak siapa? tag orang nya dong..*`, m)
  ...
  try {
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      conn.reply(m.chat,`*@${user.split('@')[0]} kaga nembak lu tlol, sadar diri dong lu itu burik,cupu,bgo,tlol.*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }else{
      global.db.data.users[user].pasangan = ""
      conn.reply(m.chat,`*@${user.split('@')[0]} di tolak ama @{m.sender.split("@") [0]}Kasian  awkoawok makanya cakep kntol:v*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
  } 
